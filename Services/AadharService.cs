...
namespace AVSBackend.Services
{
    public class AadharService : IAadharService
    {
        ...
        {
            string url = $"http://192.168.20.151:90/OVD_L/Frm.aspx?adharNo={aadharNo}";
        ...
        public async Task<(bool Success, string Message)> VerifyAadharOtpAsync(AadharVerifyRequest request)
        {
            ...
            string bankName = "avs test";

            string url = $"http://192.168.20.151:90/OVD_L/FrmVerifyOTP.aspx?Clientiid={clientiid}&ClientID={clientId}&OTP={request.Otp}&BankName={bankName}&adharNo={request.AadharNo}";
